...
! defined( 'WPCSL_VERSION' ) && define( 'WPCSL_VERSION', '1.3.6' );
	...
	function wpcsl_init() {
		...
		if ( ! class_exists( 'WPCleverWpcsl' ) && class_exists( 'WC_Product' ) ) {
			class WPCleverWpcsl {
				...
				function init() {
					// load text-domain
					load_plugin_textdomain( 'wpc-smart-linked-products', false, basename( WPCSL_DIR ) . '/languages/' );
				}
				...
				function admin_enqueue_scripts( $hook ) {
					if ( apply_filters( 'wpcsl_ignore_backend_scripts', false, $hook ) ) {
						return null;
					}
                    ...
                    <div class="wpclever_settings_page wrap">
                        ...
                        <div class="wpclever_settings_page_desc about-text">
                            <p>
                                ...
                                <br/>
                                <a href="<?php echo esc_url( WPCSL_REVIEWS ); ?>"
                                   target="_blank"><?php esc_html_e( 'Reviews', 'wpc-smart-linked-products' ); ?></a> |
                                <a href="<?php echo esc_url( WPCSL_CHANGELOG ); ?>"
                                   target="_blank"><?php esc_html_e( 'Changelog', 'wpc-smart-linked-products' ); ?></a>
                                |
                                <a href="<?php echo esc_url( WPCSL_DISCUSSION ); ?>"
                                   target="_blank"><?php esc_html_e( 'Discussion', 'wpc-smart-linked-products' ); ?></a>
                            </p>
                        </div>
                        ...
                        <div class="wpclever_settings_page_nav">
                            <h2 class="nav-tab-wrapper">
                                <a href="<?php echo esc_url( admin_url( 'admin.php?page=wpclever-wpcsl-us&tab=settings' ) ); ?>"
                                   class="<?php echo esc_attr( $active_tab === 'settings' ? 'nav-tab nav-tab-active' : 'nav-tab' ); ?>">
									<?php esc_html_e( 'Settings', 'wpc-smart-linked-products' ); ?>
                                </a>
                                <a href="<?php echo esc_url( admin_url( 'admin.php?page=wpclever-wpcsl-us' ) ); ?>"
                                   class="<?php echo esc_attr( $active_page === 'wpclever-wpcsl-us' && $active_tab !== 'premium' && $active_tab !== 'settings' ? 'nav-tab nav-tab-active' : 'nav-tab' ); ?>">
									<?php esc_html_e( 'Smart Upsells', 'wpc-smart-linked-products' ); ?>
                                </a>
                                <a href="<?php echo esc_url( admin_url( 'admin.php?page=wpclever-wpcsl-cs' ) ); ?>"
                                   class="<?php echo esc_attr( $active_page === 'wpclever-wpcsl-cs' && $active_tab !== 'premium' && $active_tab !== 'settings' ? 'nav-tab nav-tab-active' : 'nav-tab' ); ?>">
									<?php esc_html_e( 'Smart Cross-sells', 'wpc-smart-linked-products' ); ?>
                                </a>
                                <a href="<?php echo esc_url( admin_url( 'admin.php?page=wpclever-wpcsl-us&tab=premium' ) ); ?>"
							...
							if ( $active_tab === 'settings' ) {
                                ...
                                <div class="wpclever_settings_page_content_text">
                                    <p>Get the Premium Version just $29!
                                        <a href="https://wpclever.net/downloads/smart-linked-products?utm_source=pro&utm_medium=wpcsl&utm_campaign=wporg"
                                           target="_blank">https://wpclever.net/downloads/smart-linked-products</a>
                                    </p>
                            ...
                            <tr>
                                <td>
                                    ...
                                    <div class="wpcsl_add_rule">
                                        <div>
                                            <a href="#" class="wpcsl_new_rule button"
                                        ...
                                        <div>
                                            <a href="#" class="wpcsl_import_export hint--left"
                                               aria-label="<?php esc_attr_e( 'Remember to save current rules before exporting to get the latest version.', 'wpc-smart-linked-products' ); ?>"
                                               data-name="<?php echo esc_attr( $name ); ?>"
                                               style="color: #999999"><?php esc_html_e( 'Import/Export', 'wpc-smart-linked-products' ); ?></a>
                                        </div>
                                    </div>
                                </td>
                            </tr>
				...
				function ajax_import_export_save() {
					if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'wpcsl_nonce' ) || ! current_user_can( 'manage_options' ) ) {
						die( 'Permissions check failed!' );
					}
					...
					if ( ! empty( $rules ) ) {
						...
						if ( $rules !== null && ( in_array( $name, [ 'wpcsl_us', 'wpcsl_cs', 'wpcsl_rl' ], true ) ) ) {
				...
				function rule( $key = '', $name = 'wpcsl_us', $rule = [], $active = false ) {
					...
					$rule_name         = $rule['name'] ?? '';
					?>
                    <div class="<?php echo esc_attr( $active ? 'wpcsl_rule active' : 'wpcsl_rule' ); ?>"
                         data-key="<?php echo esc_attr( $key ); ?>">
                        <div class="wpcsl_rule_heading">
                            <span class="wpcsl_rule_move"></span>
                            <span class="wpcsl_rule_label"><span
                                        class="wpcsl_rule_name"><?php echo esc_html( $rule_name ); ?></span> <span
                                        class="wpcsl_rule_apply_get"><?php echo esc_html( $apply . ' | ' . $get ); ?></span></span>
                            <a href="#" class="wpcsl_rule_duplicate"
                               data-name="<?php echo esc_attr( $name ); ?>"><?php esc_html_e( 'duplicate', 'wpc-smart-linked-products' ); ?></a>
                            <a href="#"
                               class="wpcsl_rule_remove"><?php esc_html_e( 'remove', 'wpc-smart-linked-products' ); ?></a>
                        </div>
                        <div class="wpcsl_rule_content">
                            <div class="wpcsl_tr">
                                ...
                                <div class="wpcsl_td">
                                    <input type="text" class="regular-text wpcsl_rule_name_val"
                                           name="<?php echo esc_attr( $name ); ?>[<?php echo esc_attr( $key ); ?>][name]"
				...
				function source( $name = 'wpcsl_us', $key = '', $apply = 'all', $products = [], $terms = [], $combination = [], $type = 'apply', $get_limit = null, $get_orderby = null, $get_order = null ) {
                    ...
                    <div class="wpcsl_tr">
                        ...
                        <div class="wpcsl_td wpcsl_rule_td">
                            <select class="wpcsl_source_selector wpcsl_source_selector_<?php echo esc_attr( $type ); ?>"
                                ...
                                <option value="combination" <?php selected( $apply, 'combination' ); ?>
                                        disabled><?php esc_html_e( 'Combined (Premium)', 'wpc-smart-linked-products' ); ?></option>
								<?php
							...
							<?php if ( $type === 'get' ) { ?>
                                <span class="show_get hide_if_get_products">
										<span><?php esc_html_e( 'Limit', 'wpc-smart-linked-products' ); ?> <input
                                                    type="number" min="1" max="50"
                                                    name="<?php echo esc_attr( $name ); ?>[<?php echo esc_attr( $key ); ?>][get_limit]"
                                                    value="<?php echo esc_attr( $get_limit ); ?>"/></span>
										<span>
										<?php esc_html_e( 'Order by', 'wpc-smart-linked-products' ); ?> <select
                                                    name="<?php echo esc_attr( $name ); ?>[<?php echo esc_attr( $key ); ?>][get_orderby]">
										...
										<span><?php esc_html_e( 'Order', 'wpc-smart-linked-products' ); ?> <select
                                                    name="<?php echo esc_attr( $name ); ?>[<?php echo esc_attr( $key ); ?>][get_order]">
                    ...
                    <div class="wpcsl_tr hide_<?php echo esc_attr( $type ); ?> show_if_<?php echo esc_attr( $type ); ?>_products">
                        ...
                        <div class="wpcsl_td wpcsl_rule_td">
                            <select class="wc-product-search wpcsl-product-search"
                                    name="<?php echo esc_attr( $name ); ?>[<?php echo esc_attr( $key ); ?>][<?php echo esc_attr( $type . '_products' ); ?>][]"
                                    multiple="multiple"
                                    data-placeholder="<?php esc_attr_e( 'Search for a product&hellip;', 'wpc-smart-linked-products' ); ?>"
                    ...
                    <div class="wpcsl_tr show_<?php echo esc_attr( $type ); ?> hide_if_<?php echo esc_attr( $type ); ?>_all hide_if_<?php echo esc_attr( $type ); ?>_products hide_if_<?php echo esc_attr( $type ); ?>_combination">
                        ...
                        <div class="wpcsl_td wpcsl_rule_td">
                            <select class="wpcsl_terms" data-type="<?php echo esc_attr( $type ); ?>"
