...
(function ($) {
    ...
    $(document).on('change, keyup', '.wpcsl_rule_name_val', function () {
        ...
        $(this).closest('.wpcsl_rule').find('.wpcsl_rule_name').html(name.replace(/(<([^>]+)>)/ig, ''));
    });

    $(document).on('change', '.wpcsl_terms', function () {
        var $this = $(this);
        var type = $this.data('type');
        var apply = $(this).closest('.wpcsl_rule').find('.wpcsl_source_selector_' + type).val();
    ...
    $(document).on('click touch', '.wpcsl_new_combination', function (e) {
        var $combinations = $(this).closest('.wpcsl_tr').find('.wpcsl_combinations');
        var key = $(this).closest('.wpcsl_rule').data('key');
    ...
    function wpcsl_combination_terms_init() {
        $('.wpcsl_apply_terms').each(function () {
            var $this = $(this);
            var taxonomy = $this.closest('.wpcsl_combination').find('.wpcsl_combination_selector').val();
    ...
    function wpcsl_source_init(type = 'apply', $rule) {
        if (typeof $rule !== 'undefined') {
            var apply = $rule.find('.wpcsl_source_selector_' + type).find(':selected').val();
            var text = $rule.find('.wpcsl_source_selector_' + type).find(':selected').text();
    ...
    function wpcsl_build_label($rule) {
        if (typeof $rule !== 'undefined') {
            var apply = $rule.find('.wpcsl_source_selector_apply').find('option:selected').text();
            var get = $rule.find('.wpcsl_source_selector_get').find('option:selected').text();

            $rule.find('.wpcsl_rule_apply_get').html('Apply for: ' + apply + ' | Linked products: ' + get);
        } else {
            $('.wpcsl_rule ').each(function () {
                var $this = $(this);
                var apply = $this.find('.wpcsl_source_selector_apply').find('option:selected').text();
                var get = $this.find('.wpcsl_source_selector_get').find('option:selected').text();

                $this.find('.wpcsl_rule_apply_get').html('Apply for: ' + apply + ' | Linked products: ' + get);
            });
        }
    }
